...
class Scraper {
    ...
    async scrapeShowById(id) {
        try {
            ...
            const totalEpisodes = $('p:contains("Episodes")').text().replace('Episodes:', '').trim();
            const year = $('p:contains("Released")').text().replace('Released:', '').trim();
            ...
            return { title, description, episodes, cast, totalEpisodes, duration, country, status, year, genres, otherNames };;
        } catch (error) {
